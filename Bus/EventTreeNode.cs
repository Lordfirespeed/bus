...
public class EventTreeNode
{
    ...
    protected sealed class EventListenerEnumerable : IEnumerable<IEventListener>, IDisposable
    {
        ...
        {
            IEnumerable<IEventListener>? parentListeners = _node.Parent?.GetPriorityListeners(priority);
            ...
            if (parentListeners is null && thisListeners.Count == 0)
                return null;

            return (parentListeners ?? []).Concat(thisListeners);
